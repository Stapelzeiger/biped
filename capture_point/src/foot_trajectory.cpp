...
Eigen::Vector<double, 4> SplineTrajectory::get_spline_coef(double tf, double qi, double qi_dot, double qf, double qf_dot)
{
...
double SplineTrajectory::get_q(Eigen::Vector<double, 4> coeff, double t)
{
    return coeff(0) + coeff(1)*t + coeff(2)*pow(t, 2) + coeff(3)*pow(t, 3);
}

double SplineTrajectory::get_q_dot(Eigen::Vector<double, 4> coeff, double t)
{
    return coeff(1) + 2*coeff(2)*t + 3*coeff(3)*pow(t, 2);
}

double SplineTrajectory::get_q_ddot(Eigen::Vector<double, 4> coeff, double t)
{
...
void FootTrajectory::get_traj_foot_pos_vel(double T_since_begin_step, Eigen::Vector3d &foot_pos, Eigen::Vector3d &foot_vel, Eigen::Vector3d &foot_acc)
{
    ...
    if (T_lift_remaining >= 0) {
        ...
        if (T_lift_remaining > fraction*T_lift || !coeff_lift_computed_){
            coeff_z_spline_lift_ = spline_.get_spline_coef(T_lift_remaining, computed_foot_pos_(2), computed_foot_vel_(2), initial_position_(2) + lift_foot_hgt, delta_h_step / T_keep);
            coeff_lift_computed_ = true;
            coeff_lift_timestamp_ = T_lift_remaining;
        }
        pos(2) = spline_.get_q(coeff_z_spline_lift_, coeff_lift_timestamp_ - T_lift_remaining);
        vel(2) = spline_.get_q_dot(coeff_z_spline_lift_, coeff_lift_timestamp_ - T_lift_remaining);
        acc(2) = spline_.get_q_ddot(coeff_z_spline_lift_, coeff_lift_timestamp_ - T_lift_remaining);
        state_in_traj_ = 0;
        ...
        if (T_lower_remaining > fraction*T_lower || !coeff_lower_computed_){
            coeff_z_spline_lower_ = spline_.get_spline_coef(T_lower_remaining, computed_foot_pos_(2), computed_foot_vel_(2), desired_end_position_(2), -lower_foot_impact_vel);
            coeff_lower_computed_ = true;
            coeff_lower_timestamp_ = T_lower_remaining;
        }
        pos(2) = spline_.get_q(coeff_z_spline_lower_, coeff_lower_timestamp_ - T_lower_remaining);
        vel(2) = spline_.get_q_dot(coeff_z_spline_lower_, coeff_lower_timestamp_ - T_lower_remaining);
        acc(2) = spline_.get_q_ddot(coeff_z_spline_lower_, coeff_lower_timestamp_ - T_lower_remaining);
    } else
    {
        // continue lowering
    ...
    {
        coeff_x_spline_ = spline_.get_spline_coef(T_remaining, computed_foot_pos_(0), computed_foot_vel_(0), desired_end_position_(0), 0);
        coeff_y_spline_ = spline_.get_spline_coef(T_remaining, computed_foot_pos_(1), computed_foot_vel_(1), desired_end_position_(1), 0);
        pos(0) = spline_.get_q(coeff_x_spline_, 0.0);
        pos(1) = spline_.get_q(coeff_y_spline_, 0.0);
        vel(0) = spline_.get_q_dot(coeff_x_spline_, 0.0);
        vel(1) = spline_.get_q_dot(coeff_y_spline_, 0.0);
        acc(0) = spline_.get_q_ddot(coeff_x_spline_, 0.0);
        acc(1) = spline_.get_q_ddot(coeff_y_spline_, 0.0);
        coeff_xy_computed_ = true;
        coeff_xy_timestamp_ = T_remaining;
    }else{
        pos(0) = spline_.get_q(coeff_x_spline_, coeff_xy_timestamp_ - T_remaining);
        pos(1) = spline_.get_q(coeff_y_spline_, coeff_xy_timestamp_ - T_remaining);
        vel(0) = spline_.get_q_dot(coeff_x_spline_, coeff_xy_timestamp_ - T_remaining);
        vel(1) = spline_.get_q_dot(coeff_y_spline_, coeff_xy_timestamp_ - T_remaining);
        acc(0) = spline_.get_q_ddot(coeff_x_spline_, coeff_xy_timestamp_ - T_remaining);
        acc(1) = spline_.get_q_ddot(coeff_y_spline_, coeff_xy_timestamp_ - T_remaining);
    }
