...
class PosCtrl(Node):
    ...
    def odom_callback(self, msg):
        with self.lock:
            ...
            dx = robot_pos.x - goal_pos.x
            dy = robot_pos.y - goal_pos.y
            dxb = math.cos(robot_yaw)*dx + math.sin(robot_yaw)*dy
            dyb = -math.sin(robot_yaw)*dx + math.cos(robot_yaw)*dy
            dyaw = wrap_angle(robot_yaw - goal_yaw)
            vx = clamp(-self.kpos*dxb, -self.vx_max, self.vx_max)
            vy = clamp(-self.kpos*dyb, -self.vy_max, self.vy_max)
