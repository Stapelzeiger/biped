...
class MujocoImitNodeParallel:
    def __init__(self, mujoco_xml_path, num_envs, sim_time_sec=0.0001, vis_rate=60, visualize=False, ):
    ...
    def reset(self, qpos, qvel):
        ...
        # self.model.eq_data[0][0] = qpos[0]
        # self.model.eq_data[0][1] = qpos[1]
        ...
        # # Activate equality constraint
        # self.model.eq_active[0] = 1

        # # Move model down until contact
        # self.read_contact_states()
        # while not (self.contact_states['R_FOOT'] or self.contact_states['L_FOOT']):
        #     self.model.eq_data[0][2] -= 0.5 * self.dt
        #     self.read_contact_states()
        # # Contact made. Release the equality constraint
    ...
    def run_joint_controllers(self, action):
        ...
        for key, value in self.q_joints.items():
            ...
            # TODO: get values from action correctly
            value['desired_pos'] = action[self.model.jnt_qposadr[id_joint_mj]]
            value['desired_vel'] = action[self.model.jnt_dofadr[id_joint_mj]]
            if not math.isnan(action[self.model.jnt_dofadr[id_joint_mj]]):
                value['feedforward_torque'] = action[self.model.jnt_dofadr[id_joint_mj]]
            else:
                value['feedforward_torque'] = 0.0
        ...
        return self.model.qpos, self.model.qvel
