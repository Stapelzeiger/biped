...
class PolicyBC:
    def __init__(self, state_size, action_size, num_input_states):
        ...
        policy_arch = [
            {'Layer': 'Linear', 'Input': self.state_size * num_input_states, 'Output': 256},
            {'Layer': 'ReLU'},
            {'Layer': 'Linear', 'Input': 256, 'Output': 512},
            {'Layer': 'ReLU'},
            {'Layer': 'Linear', 'Input': 512, 'Output': 256},
            {'Layer': 'ReLU'},
            {'Layer': 'Linear', 'Input': 256, 'Output': self.action_size}
        ]

        self.policy = BC_Agent(policy_arch, torch.zeros(self.state_size), torch.zeros(self.state_size), torch.zeros(self.action_size), torch.zeros(self.action_size))
        self.policy.load_policy(self.folder + f"policies/{self.policy_name}.pt")

    def __call__(self, input_state):
        policy_actions = self.policy(torch.tensor(input_state, dtype=torch.float32))
